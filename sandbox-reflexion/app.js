...
const stick=$("camera-stick"),stickKnob=stick.querySelector("i");let stickPointer=null,stickVector={x:0,y:0},stickFrame=null,stickTime=0;
function moveStick(event){const rect=stick.getBoundingClientRect();stickVector={x:Math.max(-1,Math.min(1,(event.clientX-(rect.left+rect.width/2))/(rect.width*.34))),y:Math.max(-1,Math.min(1,(event.clientY-(rect.top+rect.height/2))/(rect.height*.34)))};stickKnob.style.transform=`translate(${stickVector.x*28}px,${stickVector.y*28}px)`;}
function animateStick(time){if(stickPointer==null)return;const frameScale=Math.min(2,(time-stickTime)/16.67||1);stickTime=time;const deadZone=.08,x=Math.abs(stickVector.x)<deadZone?0:stickVector.x,y=Math.abs(stickVector.y)<deadZone?0:stickVector.y;opticsScene.orbitCamera(x*.025*frameScale,y*.02*frameScale);stickFrame=requestAnimationFrame(animateStick);}
stick.addEventListener("pointerdown",event=>{stickPointer=event.pointerId;stick.setPointerCapture(stickPointer);moveStick(event);stickTime=performance.now();stickFrame=requestAnimationFrame(animateStick);});stick.addEventListener("pointermove",event=>{if(event.pointerId===stickPointer){event.preventDefault();moveStick(event);}});function releaseStick(event){if(event.pointerId!==stickPointer)return;stickPointer=null;cancelAnimationFrame(stickFrame);stickVector={x:0,y:0};stickKnob.style.transform="translate(0,0)";}stick.addEventListener("pointerup",releaseStick);stick.addEventListener("pointercancel",releaseStick);
